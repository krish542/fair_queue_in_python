...
class User:
    ...
    def send_packet(self, current_time):
        ...
        if self.deficit_counter + self.quantum >= packet_size:
            self.packets.pop(0)
            self.deficit_counter += self.quantum - packet_size
            self.total_bytes_sent += packet_size
            self.total_latency += (current_time - arrival_time)
            self.packets_sent += 1
            return packet_size
        else:
...
def calculate_throughput(users):
    total_time = max(user.total_latency / user.total_bytes_sent for user in users.values())
...
def calculate_fairness_index(throughputs):
    ...
    fairness_index = sum((throughput / total_throughput) ** 2 for throughput in throughputs.values())
